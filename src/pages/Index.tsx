...
import { Plus, Home, ChefHat, FastForward, RefreshCw } from "lucide-react";
...
const Index = () => {
  ...
  useEffect(() => {
    ...
    // Set up periodic refresh every 30 seconds to ensure data stays in sync
  ...
  const loadPantryItems = async () => {
    try {
      ...
      // Show success message if items were loaded successfully
      if (items.length > 0) {
        console.log(`✅ Loaded ${items.length} pantry items for user ${user?._id}`);
      }
    } catch (error) {
      console.error('Error loading pantry items:', error);
      toast({
        title: "Connection Issue",
        description: "Unable to load your pantry items. Your data is safe - try clicking 'Refresh Items' or reload the page.",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
    ...
    try {
      ...
      if (addedCount > 0 && updatedCount > 0) {
        toast({
          ...
          description: `Added ${addedCount} new items and updated ${updatedCount} existing items. Your items are saved to your profile!`
        });
      } else if (addedCount > 0) {
        toast({
          title: "Receipt Processed! 📄",
          description: `Added ${addedCount} new items from your receipt. Your items are saved to your profile!`
        });
      } else if (updatedCount > 0) {
        toast({
          title: "Receipt Processed! 📄",
          description: `Updated quantities for ${updatedCount} existing items. Your items are saved to your profile!`
        });
      } else {
        toast({
          title: "Receipt Processed! 📄",
            ...
            <Button
              onClick={async () => {
                try {
                  ...
                  const nonExpiredItems = pantryItems.filter(item => {
                    const expiryDate = new Date(item.expiryDate);
                  ...
                  for (const expiredItem of expiredItems) {
                    if (expiredItem._id || expiredItem.id) {
                      await apiService.deletePantryItem(expiredItem._id || expiredItem.id!);
                    }
                  }
                  
                  // Update local state
                  setPantryItems(nonExpiredItems);
                  
                  const clearedCount = expiredItems.length;
                  if (clearedCount > 0) {
                    toast({
                      title: "Items Refreshed",
                      description: `Cleared ${clearedCount} expired item${clearedCount > 1 ? 's' : ''} from your pantry.`,
                      duration: 3000
                    });
                  } else {
                    toast({
                      title: "No Expired Items",
                      description: "All your items are still fresh!",
                      duration: 2000
                    });
                  }
                } catch (error) {
                  ...
                  toast({
                    title: "Error",
                    description: "Failed to clear expired items. Please try again.",
                    variant: "destructive"
                  });
                }
              }}
              variant="outline"
              className="border-gray-300 hover:border-gray-400 text-gray-700 hover:text-gray-900 px-6 py-3 text-lg rounded-full shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-105"
            >
